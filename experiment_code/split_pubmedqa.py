...
if split_name == 'pqal':
    ...
    dataset = json.load(open('../ori_pqal.json'))

    CV_set, testset = split(dataset, 2)
    with open('../pubmedqa_test_set.json', 'w') as f:
        json.dump(testset, f, indent=4)

    tr_set, dev_set = split(CV_set, 2)
    with open('../pubmedqa_train_set.json', 'w') as f:
        json.dump(tr_set, f, indent=4)
    with open('../pubmedqa_dev_set.json', 'w') as f:
